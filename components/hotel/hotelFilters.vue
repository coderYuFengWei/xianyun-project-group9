    ...
    <div class="container">
        <el-row class="select">
            ...
            <el-col :span="4">
                ...
                <el-select v-model="filters.level" multiple collapse-tags style="margin-left: 20px;" placeholder="不限">
                    <el-option v-for="(item,index) in data.levels" :key="item.level" :label="item.name" :value="item.id">
            ...
            <el-col :span="4">
                ...
                <el-select v-model="filters.type" multiple collapse-tags style="margin-left: 20px;" placeholder="不限">
                    <el-option v-for="(item,index) in data.types" :key="item.type" :label="item.name" :value="item.id">
                    </el-option>
                </el-select>
            </el-col>
            <el-col :span="4">
                <span class="text">酒店设施</span>
                <el-select v-model="filters.asset" multiple collapse-tags style="margin-left: 20px;" placeholder="不限">
                    <el-option v-for="(item,index) in data.assets" :key="item.asset" :label="item.name" :value="item.id">
                    </el-option>
                </el-select>
            </el-col>
            <el-col :span="4">
                <span class="text">酒店品牌</span>
                <el-select v-model="filters.brand" multiple collapse-tags style="margin-left: 20px;" placeholder="不限">
                    <el-option v-for="(item,index) in data.brands" :key="item.brand" :label="item.name" :value="item.id">
                    </el-option>
                </el-select>
            </el-col>
        </el-row>
        <el-row class="center" v-for="(item,index) in hotelList">
            ...
            <el-col :span="11" class="detail">
                ...
                <el-row style="margin:15px 0px;">
                    ...
                    <el-col :span="7">
                        <span style="color:#f90">{{item.all_remarks}} </span>评论</el-col>
                    <el-col :span="8">
                        <span style="color:#f90">{{item.common_remarks}} </span>篇游记</el-col>
                </el-row>
                <p style="color:#666;font-size:16px;">
                    <i class="iconfont icon-htmal5icon14" style="font-size:25px;"></i>位于: {{item.address}}</p>
            </el-col>
...
export default {
    ...
    computed: {
        hotelList() {
            return this.hotel.slice((this.pageIndex - 1) * this.pageSize, this.pageIndex * this.pageSize)
        }
    },
    methods: {
