  ...
  <div class="scenicNav">
    <div class="cityNav" @mouseleave="hiddenCity">
      ...
      <div class="cityAttractions" v-if="isShow && cityMenuList[listIndex].children">
        <p v-for="(item,index) in cityMenuList[listIndex].children" :key="index">
          <span>{{index + 1}}</span>
          <em @click="acquiringCities(item.city)">{{item.city}}</em>
          <i @click="acquiringCities(item.city)">{{item.desc}}</i>
        </p>
      </div>
    </div>
