<template>
  <div class="container">
    <p style="margin:20px 0 0 10px" >酒店<span v-if="form.city">>{{form.city}}酒店预订</span></p>
    <div>
      <el-row style="margin:20px 0" :gutter="20">
        <el-col :span="5">
          <el-autocomplete style="width:100%" class="inline-input" v-model="form.city" :fetch-suggestions="querySearch" placeholder="目的地" :trigger-on-focus="false" @blur="handleBlur('city')" @select="handleSelect"></el-autocomplete>
        </el-col>
        <el-col :span="10">
          <el-date-picker style="width:100%" v-model="form.date" type="daterange" range-separator="至" start-placeholder="入住时间" end-placeholder="离店时间" @change="handleDate">
          </el-date-picker>
        </el-col>
        <el-col :span="5">
          <el-popover placement="bottom" width="400" trigger="click">
              ...
              <el-row :gutter="20">
                ...
                <el-col :span="9">
                  <el-select v-model="adult" placeholder="0成人">
                    <el-option v-for="item in adults" :key="item.value" :label="item.label" :value="item.label">
                    </el-option>
                  </el-select>
                </el-col>
                <el-col :span="9">
                  <el-select v-model="child" placeholder="0儿童">
                    <el-option v-for="item in childs" :key="item.value" :label="item.label" :value="item.label">
                    </el-option>
                  </el-select>
                </el-col>
              </el-row>
            ...
            <el-input v-model="val" slot="reference" placeholder="人数未定" suffix-icon="el-icon-user">
            </el-input>
          </el-popover>
        </el-col>
        <el-col :span="3">
        ...
        <el-col :span="15" class="introduction">
          <el-row>
            ...
            <el-col :span="21">
              ...
              <nuxt-link to="" v-for="(item,index) in showList" @click.native="hangletag(index)" :class="{'active':index==tag}" style="line-height:30px;">{{item.name}}</nuxt-link>
    ...
    <div v-if="!handleHotel.length" style="padding:20px; text-align: center;font-size:30px;">
      暂无符合条件的酒店
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    showList() {
      if (this.label == false) {  //当数据不需要完全显示的时候
        var showList = [];  //定义一个空数组
        if (this.street.length > 11) {  //这里我们先显示前三个
          for (var i = 0; i < 11; i++) {
